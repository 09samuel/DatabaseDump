...
function DatabasesPage() {
  ...
  return (
    <div className="flex flex-col h-full p-6 border border-black rounded-lg bg-[#f7f7f7]">
      
      {/* Top Section - same for both states */}
      <div className="flex justify-between items-center mb-8">
        {/* Title and Description */}
        <div>
          <h1 className="text-2xl font-semibold">Databases Page</h1>
      ...
      <div className="mt-8 flex flex-col flex-1 min-h-0 overflow-y-auto">
        {isLoading ? (
          ...
          <EmptyState icon= {DatabaseIcon} mainMessage={"No databases found"} subMessage={"Get started by adding your first database or connect to existing ones."}/>
        ) : isSearchEmpty ? (
          <EmptySearchState query={dbSearch} />
        ) : (
          <DatabaseList databases={filteredDatabases} onBackup={handleBackup} onEdit={handleEdit}  onDelete={handleDelete}/>
        )}
