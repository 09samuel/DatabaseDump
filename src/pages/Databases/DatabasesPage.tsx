  ...
  function DatabasesPage() {
    ...
    return (
      <div className="flex flex-col h-full min-h-0 border border-gray-200 dark:border-neutral-800 rounded-lg bg-gray-100 dark:bg-neutral-900 text-sm md:text-base text-gray-900 dark:text-gray-100 overflow-hidden">
        <div className="flex flex-col flex-1 min-h-0 p-4 overflow-hidden">
          ...
          <div className="flex flex-nowrap items-center mb-8 gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-gray-500 dark:text-gray-400">Plan, prioritize and accomplish your tasks with ease</p>
            </div>
            
            {/* Add Database Button / Skeleton */}
            <div className="shrink-0">
              {isLoading ? (
                <div className=" h-10 w-10 md:h-auto md:w-auto gap-4 leading-none bg-gray-200 dark:bg-neutral-800 rounded-full animate-pulse self-end md:self-auto" />
              ) : status === "loaded" || status === "empty" ? (
                <button 
                  onClick={() => setIsAddOpen(true)} 
                  className="flex items-center justify-center h-10 w-10 md:h-auto md:w-auto gap-4 md:px-6 md:py-3 leading-none bg-blue-600 text-white rounded-full text-sm font-medium self-end md:self-auto"
                >
            ...
            { isLoading ? (
              <div className="space-y-3 overflow-y-auto pb-24">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-gray-200 dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-800 shadow-sm animate-pulse h-24 w-full"
                  />
                ))}
              </div>
            ) : error ? (
